...
const FOCUSED_ELEMENT_KEY = "visual_edit_focused_element" as const;

// Deduplicate helper for high-frequency traffic (HIT / FOCUS_MOVED / SCROLL)
// -----------------------------------------------------------------------------
let _visualEditLastMsg = "";
const postMessageDedup = (data: any) => {
  try {
    const key = JSON.stringify(data);
    if (key === _visualEditLastMsg) return; // identical – drop
    _visualEditLastMsg = key;
  } catch {
...
export default function HoverReceiver() {
  ...
  const handleStyleChange = (
    ...
    appliedStylesRef.current.set(visualEditId, {
      ...existingStyles,
      ...styles,
    });
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const tagName =
          hit.getAttribute("data-visual-edit-name") ||

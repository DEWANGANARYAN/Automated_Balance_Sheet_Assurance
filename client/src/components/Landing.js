          ...
          <tbody>
            {reviews.map((r) => (
              <tr key={r._id}>
                ...
                <td>
                  {r.status === "waiting" ? (
                    <button onClick={() => openModal(r)}>Submit</button>
                  ) : (
                    <span>{r.status === "submitted" ? "📤 Submitted" : r.status}</span>
                  )}
                </td>
              </tr>
            ))}
      ...
      {showModal && (
        <div style={modal.overlay}>
          <div style={modal.content}>
            <h3>Submit Review Proof for {activeReview.gl_code}</h3>
            <textarea
